...
<?php	
		...
		printf("<tr><td>№</td><td>Категория</td><td>Наименование</td><td>Наличие</td><td>Цена</td></tr>");
...
?>
	...
	<?php	
		...
		printf("<form>");
		printf("<table id='tabl2' border = '2'>");
		printf("<tr><td>№</td<td>ФИО</td><td>Логин</td><td>Статус</td></tr>");
		while($row2 = mysqli_fetch_assoc($sel2))
		{
			$url2 = "delete_log.php?product=" .$row2['id'];
			$url3 = "update_log.php?product=" .$row2['id'];
			
			printf("<tr><td>%d</td>><td>%s</td><td>%s</td><td>%s</td><td><a href = %s>Изменить</a></td><td><a href =  %s>Удалить</a></td></tr>", $row2['id'], $row2['name'], $row2['login'], $row2['status'], $url3, $url2);
		}	
?>
	<p>Пользователи</p>
<?php	
		...
		printf("<form>");
		printf("<table id='tabl3' border = '2'>");
		printf("<tr><td>№</td><td>Тип заказчика</td><td>Наименование/ФИО</td><td>Адрес</td><td>Телефон</td></tr>");
		while($row3 = mysqli_fetch_assoc($sel3))
		{
			$url4 = "delete_cust.php?product=" .$row3['id'];
			$url5 = "update_cust.php?product=" .$row3['id'];
			
			printf("<tr><td>%d</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td><a href = %s>Изменить</a></td><td><a href =  %s>Удалить</a></td></tr>", $row3['id'], $row3['type_cus'], $row3['name_cus'], $row3['adres'], $row3['phone'], $url5, $url4);
		}	
?>
	<p>Заказчики</p>
</body>